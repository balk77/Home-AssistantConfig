...
class zolderpseudothermostat(hass.Hass):
    ...
    def restore_temperature(self, kwargs):
        ...
        if zolder_beweging == "on":
            afternoon_sp = self.args['temp_high_sp']
        else:
            afternoon_sp = self.args['temp_low_sp']
        ...
        if workday == "on":
            if self.now_is_between(self.args['week_morning_switch'],self.args['week_afternoon_switch']):
                ...
                self.log("restore to: "+str(self.args['temp_low_sp']))
                self.call_service("climate/set_temperature", entity_id=self.args['frontend_climate_device'], temperature=self.args['temp_low_sp'])
                self.call_service("input_number/set_value", entity_id=self.args['input_number_setpoint'], value=self.args['temp_low_sp'])
        else:
            if self.now_is_between(self.args['weekend_morning_switch'],self.args['weekend_afternoon_switch']):
                ...
                self.log("restore to: "+str(self.args['temp_low_sp']))
                self.call_service("climate/set_temperature", entity_id=self.args['frontend_climate_device'], temperature=self.args['temp_low_sp'])
                self.call_service("climate/set_temperature", entity_id=self.args['raw_climate_device'], temperature=self.args['temp_low_sp'])
                self.call_service("input_number/set_value", entity_id=self.args['input_number_setpoint'], value=self.args['temp_low_sp'])
    ...
    def zolder_motion(self, entity, attribute, old, new, kwargs):
        ...
        if override == "off":
            if zolder_beweging == "on":
                frontend_sp = self.args['temp_high_sp']
            else:
                frontend_sp = self.args['temp_low_sp']
